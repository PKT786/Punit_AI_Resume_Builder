...
TEMPLATES = {
    ...
    "premium": [
        ...
        {
            "id": "premium-10", "name": "Onyx Sidebar", "tier": "premium",
            "header_mode": "side-accent",
            "font_head": "Cambria", "font_body": "Calibri",
            "navy": "1A1A1A", "accent": "9C7A2E", "darkgray": "232323", "midgray": "595959",
        },
        {
            "id": "premium-11", "name": "Crimson Sidebar", "tier": "premium",
            "header_mode": "side-accent",
            "font_head": "Georgia", "font_body": "Calibri",
            "navy": "7A2323", "accent": "C9A227", "darkgray": "262626", "midgray": "595959",
        },
        {
            "id": "premium-12", "name": "Slate Sidebar", "tier": "premium",
            "header_mode": "side-accent",
            "font_head": "Cambria", "font_body": "Calibri",
            "navy": "2B3A4A", "accent": "3E8E8E", "darkgray": "232323", "midgray": "555555",
        },
        {
            "id": "premium-13", "name": "Forest Sidebar", "tier": "premium",
            "header_mode": "side-accent",
            "font_head": "Georgia", "font_body": "Calibri",
            "navy": "1E4620", "accent": "B08D57", "darkgray": "232323", "midgray": "555555",
        },
    ],
}


def pick_random_template(tier: str, exclude_ids=None) -> dict:
    """
    Random template from `tier`. If `exclude_ids` is given, those template
    ids are avoided as long as at least one other option remains in the
    tier — this is how the app avoids handing the user the same template
    twice in a row.
    """
    ...
    if exclude_ids:
        ...
        filtered = [t for t in pool if t["id"] not in exclude_ids]
        if filtered:
            pool = filtered
...
class ResumeBuilder:
    ...
    def add_header(self, data):
        ...
        elif self.header_mode == "side-accent":
            # A bold left-hand color bar through the whole header block,
            # name/title/contact left-aligned — evokes the "sidebar" look
            # of two-column resume designs while staying single-column and
            # ATS-safe (no tables, nothing that breaks reading order).
            p_name = self.doc.add_paragraph()
            p_name.paragraph_format.left_indent = Pt(22)
            p_name.paragraph_format.space_before = Pt(4)
            p_name.paragraph_format.space_after = Pt(2)
            _border_paragraph(p_name, left={"color": self.style["navy"], "sz": 42, "space": 12})
            self._run(p_name, name, font=self.font_head, size=25, color=self.navy, bold=True, spacing=0.6)

            if title:
                p_title = self.doc.add_paragraph()
                p_title.paragraph_format.left_indent = Pt(22)
                p_title.paragraph_format.space_after = Pt(6)
                _border_paragraph(p_title, left={"color": self.style["navy"], "sz": 42, "space": 12})
                self._run(p_title, title, font=self.font_body, size=11.5, color=self.accent, bold=True, spacing=0.4)

            p_contact = self.doc.add_paragraph()
            p_contact.paragraph_format.left_indent = Pt(22)
            p_contact.paragraph_format.space_after = Pt(14)
            _border_paragraph(
                p_contact,
                left={"color": self.style["navy"], "sz": 42, "space": 12},
                bottom={"color": self.style["accent"], "sz": 10, "space": 8},
            )
            self._run(p_contact, "   |   ".join(contact_bits), size=9.5, color=self.midgray)
            if data.get("linkedin"):
                self._run(p_contact, "   |   ", size=9.5, color=self.midgray)
