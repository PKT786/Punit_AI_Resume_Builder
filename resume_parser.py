...
# A single date token: "Apr 2025", "2018-07", "07/2018", "07-2018", or a bare "2018"
_DATE_TOKEN = rf"(?:{_MONTH}\s*\d{{4}}|\d{{4}}[-/]\d{{1,2}}|\d{{1,2}}[-/]\d{{4}}|\d{{4}})"
# An open-ended end date: "Present", "Current", "Till Date", "Ongoing", "Now"
_OPEN_TOKEN = r"(?:Present|Current|Till\s*Date|Ongoing|Now)"
# Separator between the two dates: hyphen/en-dash/em-dash or the word "to"
_SEP = r"\s*(?:-|–|—|to)\s*"

DATE_RANGE_RE = re.compile(
    rf"({_DATE_TOKEN}){_SEP}({_DATE_TOKEN}|{_OPEN_TOKEN})",
    re.IGNORECASE,
)
...
def _parse_experience_block(block_lines):
    ...
    Two safety nets so content is never silently dropped, even for resume
    layouts (e.g. two-column/sidebar exports) our line-based heuristic can't
    fully make sense of:
      - lines seen before the first recognizable date range are buffered and
        attached to the first job we do find, instead of being discarded.
      - if no date range is found anywhere in the block, the whole block is
        returned as a single job entry (role/company left blank for the user
        to fill in) rather than returning nothing.
    """
    jobs = []
    current = None
    pending = []  # lines seen before we've found any date range yet
    ...
    for raw in block_lines:
        ...
        if m:
            ...
            if pending:
                current["bullets"].extend(pending)
                pending = []
        else:
    ...
    if not jobs and pending:
        # No date range matched anywhere in this block — rather than losing
        # the content, keep it as a single job entry the user can relabel.
        jobs.append({
            "role": "", "company": "", "dates": "", "project": "",
            "bullets": pending,
        })
