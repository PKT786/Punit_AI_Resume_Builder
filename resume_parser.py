...
def _parse_skills_block(block_lines):
    ...
    for line in block_lines:
        ...
        if ":" in line:
            ...
            skills.append({"label": "Skills", "value": line})
    return skills
...
def _parse_experience_block(block_lines):
    ...
    for raw in block_lines:
        ...
        if m:
            ...
            role, company = (header_text.split("|", 1) + [""])[:2] if "|" in header_text \
                else (header_text.split(" - ", 1) + [""])[:2] if " - " in header_text \
                else (header_text, "")
            current = {
                "role": role.strip() or header_text,
